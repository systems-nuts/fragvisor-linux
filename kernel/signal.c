	...
	if (distributed_process(current) && sig >= SIGKVMEXIT) {
		...
		SIGVPRINTK("native %s(): [%d] "
					"base %d - %d %d %d %d %d %d %d\n",
						...
						SIGKVMPAUSEVANILLA,
						SIGKVMCPUMIGRATE,
						SIGKVMCPUMIGRATE_BACK,
						SIGKVMCPUMIGRATE_W_STATE,
						SIGKVMFTCKPT
						);
		if (sig == SIGKVMCPUMIGRATE_BACK_W_STATE) {
			SIGVPRINTK("native %s(): [%d] sig %d [[[[ CKPT ]]]] + 0\n",
						__func__, current->pid, sig);
		}
		if (sig == SIGKVMFTRESTART) {
			SIGVPRINTK("native %s(): [%d] sig %d [[[[ RESTART ]]]] + 0\n",
						__func__, current->pid, sig);
		}
		if (sig - 2 == SIGKVMCPUMIGRATE_BACK_W_STATE) {
			SIGVPRINTK("native %s(): [%d] sig %d [[[[ CKPT ]]]] - 2\n",
						__func__, current->pid, sig);
		}
		if (sig - 2 == SIGKVMFTRESTART) {
			SIGVPRINTK("native %s(): [%d] sig %d [[[[ RESTART ]]]] - 2\n",
						__func__, current->pid, sig);
		}
	}
		...
		if (distributed_remote_process(current)) {
			...
			for (j = VCPU_FD_BASE; j < MAX_POPCORN_VCPU; j++) {
				...
				// hype_node_info[from_nid][VCPU_FD_BASE + from_nid]->origin_pid; // this only saved in origin.....
					old_pid = pid;
					SIGVPRINTK("native? [convert] %s(): pid %d -> %d\n",
			...
			SIGVPRINTK("native? %s(): [%d] converting "
...
int pophype_do_send_specific_at_remote(pid_t tgid, pid_t pid, int sig, struct siginfo *info)
{
	...
	if (p && (tgid <= 0 || task_tgid_vnr(p) == tgid)) {
		error = check_kill_permission(sig, info, p);
		SIGVPRINTK("%s(): pid %d permission ret error %d\n",
						__func__, pid, error);
		/*
		 * The null signal is a permissions and process existence
		 * probe.  No signal is actually delivered.
