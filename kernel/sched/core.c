...
/*#ifdef CONFIG_POPCORN_HYPE
	 ...
	 bind process to virtio queue
	if (cpumask_weight(new_mask) == 1) {
		p->virtio_q = cpumask_first(new_mask);
#ifdef POPCORN_VIRTIO_DEBUG
		trace_printk("virtio: sched_setaffinity: process %s bound to q=%d", p->comm, p->virtio_q);
#endif
	}
#endif
#endif*/
...
SYSCALL_DEFINE2(popcorn_migrate, int, nid, void __user *, uregs)
{
	...
	if (nid < 0 || ((nid >= MAX_POPCORN_NODES)
		...
		PSPRINTK("  [%d] popcorn: invalid migration destination %d\n",
				current->pid, nid);
	//	BUG();
		return -EINVAL;
	}
