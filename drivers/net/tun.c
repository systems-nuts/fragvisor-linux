...
/* printing sk_buff info to debug tun flows
 * direction = 0 means Tx
 * direction = 1 measn Rx
*/
static inline bool pophype_print_skb(struct sk_buff *skb,
									 u32 rxhash,
									 u16 queue,
									 bool direction,
									 bool print)
{
    ...
    src_ip = (u32) ip_header->saddr;
    dest_ip = (u32) ip_header->daddr;

	tcp_header = (struct tcphdr *) skb_transport_header(skb);
	src_port = (u16) ntohs(tcp_header->source);
	dest_port = (u16) ntohs(tcp_header->dest);

	/* show only communications from/to 10.0.0.0/8 */
    snprintf(sip_str, 16, "%pI4", &src_ip);
    snprintf(dip_str, 16, "%pI4", &dest_ip);

    if ((!strcmp(sip_str, "10.4.4.222") && dip_str[0] == '1' && dip_str[1] == '0' && dip_str[2] == '.') ||
            (!strcmp(dip_str, "10.4.4.222") && sip_str[0] == '1' && sip_str[1] == '0' && sip_str[2] == '.')) {
            if (print) {
                VHOSTPKTUN("%s: %s:%u -> %s:%u, rxhash=%x, queue=%u\n",
                    direction ? "Rx" : "Tx", sip_str, src_port, dip_str, dest_port, rxhash, queue);
            }
            return true;
    }
    return false;
			    ...
			    void *accel_priv, select_queue_fallback_t fallback)
{
	...
	if (txq) {
		...
		if (e) {
			...
			txq = e->queue_index;
#ifdef POPCORN_TUN_DEBUG_RX
			if (pophype_print_skb(skb, txq2, txq, 1, 1))
				VHOSTPKTUN("tun_select_queue: flow exists! txq=%d\n", txq);
#endif /* POPCORN_TUN_DEBUG_RX */
		} else
#ifdef CONFIG_POPCORN_HYPE
		{
			txq = 0;
#ifdef POPCORN_TUN_DEBUG_RX
			if (pophype_print_skb(skb, txq2, txq, 1, 1))
				VHOSTPKTUN("tun_select_queue: flow does NOT exists! txq=%d\n", txq);
#endif /* POPCORN_TUN_DEBUG_RX */
		}
#else
...
//#if 1 // DEBUGGING
	...
	if (skb_queue_len(&tfile->socket.sk->sk_receive_queue) * numqueues
			  >= dev->tx_queue_len) {
		VHOSTPKTUN("start_xmit: packet dropped!!\n");
		goto drop;
	}
			    ...
			    int noblock)
{
	...
	if ((tun->numqueues != 1) && !tfile->detached) {
		if (pophype_print_skb(skb, rxhash, tfile->queue_index, 0, 1))
			VHOSTPKTUN("tun_get_user: flow update! txq=%d\n", tfile->queue_index);
	}
#endif
