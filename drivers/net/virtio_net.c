...
#define start_xmit virtio_start_xmit
#define virtnet_poll virtio_virtnet_poll
...
static void skb_recv_done(struct virtqueue *rvq)
{
	...
	VIRTIOPKIRQ("skb_recv_done: callback=%p, vq=%p, q=%d\n", skb_recv_done, rvq, rvq->index);

	/* Schedule NAPI, Suppress further interrupts if successful. */
	if (napi_schedule_prep(&rq->napi)) {
		virtqueue_disable_cb(rvq);
		VIRTIOPKIRQ("skb_recv_done: disabled vq, napi scheduled,  q=%u, smp=%u\n", rvq->index, smp);
		__napi_schedule(&rq->napi);
	} else {
		VIRTIOPKIRQ("skb_recv_done: no napi scheduled,  q=%u, smp=%u\n", rvq->index, smp);
	}
}
...
static int virtnet_receive(struct receive_queue *rq, int budget)
{
	...
	VIRTIOPKRX("virtnet_recv: received packets q=%d, smp=%d, recvd:%d\n", rq->vq->index, smp_processor_id(), received);
	return received;
}

int virtio_virtnet_poll(struct napi_struct *napi, int budget)
{
	struct receive_queue *rq =
...
#endif
	r = virtqueue_add_outbuf(sq->vq, sq->sg, num_sg, skb, GFP_ATOMIC);
	VIRTIOPK("xmit_skb: return=%d\n", r);

	return r;
}

netdev_tx_t virtio_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	struct virtnet_info *vi = netdev_priv(dev);
	...
	if (kick || netif_xmit_stopped(txq)) {
		int r;
		r = virtqueue_kick(sq->vq);
		VIRTIOPKTX("start_xmit: kick=%d\n", r);
	}
...
static void virtnet_set_affinity(struct virtnet_info *vi)
{
	...
	for_each_online_cpu(cpu) {
		...
		VIRTIOPK("vq_affinity: qp=%d, smp=%d\n", i, cpu);
...
static inline u16 virtnet_select_queue(struct net_device *dev,
								struct sk_buff *skb,
								void *accel_priv,
								select_queue_fallback_t fallback)
{
       /* all flows are assigned to the vcpu on
        * which they are running
        */
	u16 queue = raw_smp_processor_id() % dev->real_num_tx_queues;
	//u16 queue = 0;
#ifdef POPCORN_VIRTIO_DEBUG
	trace_printk("virtnet_select_queue: comm=%s, queue=%d\n", current->comm, queue);
#endif
    return queue;
}
#endif
...
static int virtnet_probe(struct virtio_device *vdev)
{
	...
	VIRTIOPK("virtnet_probe: vi->curr_queue_pairs=%u, vi->max_queue_pairs=%d\n", vi->curr_queue_pairs, vi->max_queue_pairs);
