...
#ifdef CONFIG_POPCORN_HYPE
	if (my_nid) {
		worker->at_remote = true;
		/* inherit origin nid and pid from lkvm process */
		worker->origin_nid = __hype_gctx.tsk->origin_nid;
		worker->origin_pid = __hype_gctx.tsk->origin_pid;
	}
#endif
	dev->worker = worker;
