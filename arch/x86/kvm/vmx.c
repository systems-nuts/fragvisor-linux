...
*/
	...
	case MSR_IA32_FEATURE_CONTROL:
		PHMIGRATEPRINTK("\t\t\t\t<%d> %s(): msr_info->index %x MSR_IA32_FEATURE_CONTROL\n",
				vcpu->vcpu_id, __func__, msr_info->index);
...
#if defined(CONFIG_POPCORN_HYPE) && defined(CONFIG_X86_64)
	PHMIGRATEPRINTK("<%d> %s(): set vcpu->arch.efer %llx -> %llx is_long_mode(&vmx->vcpu) %d\n",
			vcpu->vcpu_id, __func__, vcpu->arch.efer, efer, is_long_mode(&vmx->vcpu));
#endif
	/*
